 ...
 * TableHeader must be read from the JTable client property (see code), since
 * JTable.getTableHeader() cannot be used.
 */
...
/**
 * An alternative TableHeader that allows header cells to span multiple columns.
 * This is just a bare-bones prototype, things to do are: - Optimize paint code
 * to paint only concerned columns - Area conversion (columnAtPoint, headerRect
 * or similar) - getToolTipText(MouseEvent) - resizing of columns (which of the
 * columns should be resized?) - moving of columns (which cannot be done in real
 * time since JTable doesn't know that it has to paint multiple dragged
 * columns.) - how to serialize if the renderer isn't serializable? -
 * Accessibility
 *
 * Note: JTable aggressively installs its JTableHeader in the column header view
 * of JScrollPane, even if it is set to 'null'. To avoid confusion, override its
 * configureEnclosingScrollPane() method to do nothing, for example.
 *
 ...
 *
 */
public class TableHeader extends JComponent {
	
	/** Serial ID */
	private static final long serialVersionUID = -5738956044014391707L;

	/** TableColumn that also has a headerSpan property. */
	public static class XTableColumn extends TableColumn {
		
		/** Serial ID */
		private static final long serialVersionUID = -1819115218586609272L;
		...
		/**
		 * set 'headerSpan'. requires newHeaderSpan > 0. During the manipulation of
		 * TableColumnModels, there may be times where a column spans more columns than
		 * there are at the right of it. This state is only allowed during such
		 * modifications. Once they are finished, all spans must not be too large.
		 * XTableColumns that are hidden by the spans of their predecessors are ignored
		 * (in the TableHeader, of course not by the JTable).
		 *
		 * Due to the broken notification way of TableColumnModel/Table- Column there is
		 * no way to notify the header that it must repaint(). If firePropertyChange
		 * were not private, we could send the following fake event
		 * firePropertyChange("width", getWidth(), getWidth()); which would handle that
		 * (only width and preferred width changes can reach the header). The moral is:
		 * don't change spans later, or if, repaint the header manually.
		 * 
		 * @param newHeaderSpan new header span 
		 */
	 ...
	 * Constructor of the class.
	 * 
	 * @param table reference on the table
	 */
	 ...
	 * Set the table
	 * 
	 * @param t reference on the table
	 */
	 ...
	 * @param r reference on the rennderer
	 */
	 ...
	 * get the renderer
	 * 
	 * @param c reference on the table column
	 * 
	 * @return renderer
	 */
